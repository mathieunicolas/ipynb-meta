...
<template>
    ...
    <UFormGroup label="Taille de l'extrait" description="Modifier la taille de l'extrait des cellules, pour faciliter leur identification ou gagner de la place.">
    ...
    <UButtonGroup>
        <UButton variant="soft" label="editable" @click="row.metadata.editable = !row.metadata.editable" :color="row.metadata.editable ? 'green': 'red'" :class="row.metadata.editable ? '':'line-through'" />
        <UButton variant="soft" label="deletable" @click="row.metadata.deletable = !row.metadata.deletable" :color="row.metadata.deletable ? 'green': 'red'" :class="row.metadata.deletable ? '' : 'line-through'" />
    </UButtonGroup>
    </div>
    </template>
